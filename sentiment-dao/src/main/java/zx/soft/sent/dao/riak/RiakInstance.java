...
public class RiakInstance {
	...
	public RiakInstance() {
		try {
			...
			List<RiakNode> nodes;
			try {
				nodes = RiakNode.Builder.buildNodes(builder, Arrays.asList(ips));
				// 法定数要多于一半的节点
	...
	public void writeString(String bucketType, String bucketName, String key, String data) {
		try {
			Location location = new Location(new Namespace(bucketType, bucketName), key);
			RiakObject riakObject = new RiakObject().setContentType("text/json").setValue(BinaryValue.create(data));
			StoreValue store = new StoreValue.Builder(riakObject).withLocation(location)
					.withOption(Option.W, new Quorum(QUORUM_SIZE)).build();
			logger.info("Write a record @ :/buckets/" + bucketName + "/keys/" + key);
			client.execute(store);
	...
	public RiakObject readString(String bucketType, String bucketName, String key) {
		try {
			...
			logger.info("read a record @ :/buckets/" + bucketName + "/keys/" + key);
	...
	public void deleteObject(String bucketType, String bucketName, String key) {
		try {
			...
			logger.info("delete a record @ :/buckets/" + bucketName + "/keys/" + key);
			client.execute(dv);
		} catch (ExecutionException | InterruptedException e) {
			logger.error("Exception:{}", LogbackUtil.expection2Str(e));
			// 注意：项目稳定时，需要把抛出异常去掉，防止因个别异常线程停止
	...
	public static void main(String[] args) {
		...
		//		cluster.close();
		cluster.deleteObject("default", "0", "2015-06-28,17");
		cluster.close();
	}
